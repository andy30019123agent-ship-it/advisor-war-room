...
def _light_codes(f: str, t: str, c: str) -> List[str]:
    out = []
    out.append("trend_ok" if t == "green" else "trend_weak" if t == "red" else "trend_ok")
    if t == "amber":
        out[-1] = "trend_ok"
...
def _risk_note(codes, defense_price):
    parts = []
...
def build_primary_and_context(*, price, lights, lights_facts, valuation, rr,
    ...
    primary = {
        ...
        "risk_note": _risk_note(codes, defense_price),
        "position": pos,
        "defense_price": defense_price,
        "entry_condition": entry_condition if action in ("試單", "觀望") else None,
