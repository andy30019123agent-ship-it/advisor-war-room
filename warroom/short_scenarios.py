...
機率校正覆蓋（warroom/scenario_calibration.py 的姊妹管線，規格見該檔模組頂端說明）：
機率查表結果若在 data/prob_calibration.json 有該 bucket（技術燈×籌碼燈）的 adjusted
值，就用它取代內建 _PROB_TABLE 的規則表值，後續大盤/防守/突破/籌碼修正項照舊疊加在
adjusted 值上（不是整組跳過修正）。預設（calibration=None）會即時讀該檔案，讀不到／
壞檔／查無該 bucket 一律靜默退回內建規則表（唯讀快取，非關鍵路徑，不需 fail-closed；
跟 scenario_calibration.py 寫入 log 那條「壞檔要 fail-closed」是不同語意）。測試可傳
明確的 calibration dict 保持純函式可測（見 _resolve_probs）。
"""
import json
import os
...
# 與 warroom/scenario_calibration.py 的 CALIBRATION_PATH 同一份檔案，個別檔案各自
# 宣告字面路徑常數（跟 forecast_log／recommendation_log 兩個既有管線同款寫法，見
# warroom/build_snapshots.py 與 warroom/track_record.py 對照）。
...
def _load_calibration(path: str = CALIBRATION_PATH) -> Dict:
    """讀機率校正覆蓋表。缺檔／壞檔一律回 {}（唯讀快取，非關鍵路徑，見模組頂端說明，
    不同於 scenario_calibration.py 寫入 log 那條需要 fail-closed 的路徑）。"""
    try:
...
def _resolve_probs(technical_color: Optional[str], chips_color: Optional[str],
                   calibration: Optional[Dict] = None
                   ) -> Tuple[float, float, float, Optional[int]]:
    """回 (base, risk, bull, calibration_n)：calibration_n=None 代表用內建規則表；
    否則代表用了 data/prob_calibration.json 的 adjusted 值，calibration_n＝該 bucket
    的回填樣本數（供 build_short_scenarios 動態組 prob_note）。
    calibration=None（呼叫端未明確傳入，即時執行的正常路徑）→ 即時讀檔；測試傳明確
    dict（含 {} 代表『確定沒有覆蓋』）保持純函式可測。"""
    base, risk, bull = _prob_lookup(technical_color, chips_color)
    if calibration is None:
        calibration = _load_calibration()
    t = technical_color if technical_color in _COLOR_KEY else "yellow"
    c = chips_color if chips_color in _COLOR_KEY else "yellow"
    bucket = f"{t}_x_{c}"
    entry = calibration.get(bucket) if isinstance(calibration, dict) else None
    adjusted = entry.get("adjusted") if isinstance(entry, dict) else None
    if isinstance(adjusted, dict) and all(k in adjusted for k in ("base", "risk", "bull")):
        return float(adjusted["base"]), float(adjusted["risk"]), float(adjusted["bull"]), entry.get("n")
    return base, risk, bull, None
...
def build_short_scenarios(
    *,
    ...
    calibration：機率校正覆蓋表（見模組頂端「機率校正覆蓋」說明）。None（預設）＝
    正常執行路徑，即時讀 data/prob_calibration.json；傳明確 dict（含 {}）保持純函式
    可測、不觸碰檔案系統。"""
    if halted:
    ...
    # 機率：查表（可被 data/prob_calibration.json 的 adjusted 覆蓋）→ 修正項 →
    # clip/normalize/整數化
    base_p, risk_p, bull_p, calib_n = _resolve_probs(technical_color, chips_color, calibration)
    defense_broken = price < defense_price
    ...
    prob_note = (PROB_NOTE if calib_n is None else
                f"機率為規則估計＋歷史校正（樣本 {calib_n} 筆），不是統計勝率，更不是保證。")
    return {
        "status": "ok",
        "horizon": HORIZON,
